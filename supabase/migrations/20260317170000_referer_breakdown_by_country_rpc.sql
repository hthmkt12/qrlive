-- Add get_link_click_detail_v3 RPC with real referer breakdown per country
-- Keeps totals, day series, and country breakdown behavior from v2 unchanged
-- Filters referer_breakdown by date range and optional country_code

create or replace function public.get_link_click_detail_v3(
  p_link_id uuid,
...
as $$
  ...
  selected_country as (
    select nullif(trim(coalesce(p_country_code, '')), '') as country_code
  ),
  totals as (
    select
  ...
  referer_breakdown as (
    ...
    from (
      select
        ...
        count(*)::bigint as clicks
      from link_clicks as click_events
      cross join selected_country
      where click_events.created_at::date between p_start_date and p_end_date
        and (
          selected_country.country_code is null
          or click_events.country_code = selected_country.country_code
        )
...
grant execute on function public.get_link_click_detail_v3(uuid, date, date, text) to authenticated;
